...
(function ($, _, Backbone, JSON, AjaxManager) {
    ...
    var generateBackboneCollection = function (config) {
        ...
        collection.bind("fetching", function () {
            if ($(".grid-container").height() > 10) {
    ...
    Backbone.Grid = Backbone.View.extend({
        ...
        initialize: function () {
            ...
            if (!valid.success) {
                _.each(valid.errors, function (err) {
                    view.log(err);
                });
            }
            ...
            this.options.columns = _.isEmpty(this.options.columns) ? [
                {name: 'empty-grid', title: $.i18n.prop('js.grid.emptyColumnName')}
            ...
            this.collection.bind("fetched", function () {
                view.hideSpinner();
            });
        ...
        ***************************************************************************************************/
        ...
        setupScrolling: function () {
            ...
            if (this.options.widthType != "percentage") {
                var widths = _.reduce($("th", this.table), function (memo, it) {
                    return memo + $(it).width()
                }, 0);
        ...
        toggleColumnVisibility: function (name, visible, quiet) {
            var column = _.find(this.columns, function (it) {
                return it.name == name;
            });
        ...
        getColumnState: function () {
            ...
            var firstNoneZero = _.find(cols, function (it) {
                return it.width > 0;
            });
            ...
            _.each(cols, function (it) {
                var c = _.find(view.columns, function (column) {
